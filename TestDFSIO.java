 ...
 * <p>
 * This test writes into or reads from a specified number of files. Number of
 * bytes to write or read is specified as a parameter to the test. Each file is
 * accessed in a separate map task.
 * <p>
...
public class TestDFSIO implements Tool {
  ...
  private enum TestType {
    TEST_TYPE_READ("read"), TEST_TYPE_WRITE("write"),
  ...
  private void createControlFile(FileSystem fs, long nrBytes, // in bytes
    ...
    LOG.info(
        "creating control file: " + nrBytes + " bytes, " + nrFiles + " files");
    final int maxDirItems
        = config.getInt(DFSConfigKeys.DFS_NAMENODE_MAX_DIRECTORY_ITEMS_KEY,
    ...
    for (int i = 0; i < nrFiles; i++) {
      ...
      try {
        writer = SequenceFile.createWriter(fs, config, controlFile, Text.class,
  ...
  public static class WriteMapper extends IOStatMapper {
    ...
    public Long doIO(Reporter reporter, String name, long totalSize // in bytes
      ...
      // write to the file
      long nrRemaining;
      for (nrRemaining = totalSize; nrRemaining > 0;
    ...
    public Long doIO(Reporter reporter, String name, long totalSize // in bytes
      ...
      while (actualSize < totalSize) {
        ...
        if (curSize < 0)
          break;
        actualSize += curSize;
        reporter.setStatus("reading " + name + "@" + actualSize + "/"
            + totalSize + " ::host = " + hostName);
      }
  ...
  /**
   * Mapper class for random reads. The mapper chooses a position in the file
   * and reads bufferSize bytes starting at the chosen position. It stops after
  ...
  private void sequentialTest(FileSystem fs, TestType testType, long fileSize, // in
                                                                               // bytes
      int nrFiles) throws IOException {
  ...
  private boolean checkErasureCodePolicy(String erasureCodePolicyName,
    ...
    if (!isValid) {
      System.out
          .println("Invalid erasure code policy: " + erasureCodePolicyName);
  ...
  void createAndEnableECOnPath(FileSystem fs, Path path) throws IOException {
    ...
    for (ErasureCodingPolicyInfo info : list) {
      ...
      if (erasureCodePolicyName.equals(ec.getName())) {
        ...
        LOG.info("enable erasureCodePolicy = " + erasureCodePolicyName + " on "
            + path.toString());
        break;
      }
    }
